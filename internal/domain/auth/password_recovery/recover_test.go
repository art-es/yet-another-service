...
import (
	...
	"github.com/art-es/yet-another-service/internal/domain/auth/password_recovery/mock"
...
func TestRecover(t *testing.T) {
	for _, tt := range []struct {
		...
		},
		{
			...
			assert: func(t *testing.T, err error, state recoverState) {
				assert.ErrorIs(t, err, errorsd.ErrUserPasswordRecoveryNotFound)
				assert.False(t, state.txRollbacked)
				assert.False(t, state.txCommitted)
			},
		},
		...
		},
		{
			...
			setup: func(m recoverMocks) {
				m.expectFindRecovery(true, nil)
				m.expectFindUser(true, nil)
				m.expectCheckOldPasswordHash(errorsd.ErrHashMismatched)
			},
			assert: func(t *testing.T, err error, state recoverState) {
