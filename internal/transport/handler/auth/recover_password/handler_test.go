...
import (
	...
	"github.com/art-es/yet-another-service/internal/transport/handler/auth/recover_password/mock"
)
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			setup: func(authSvc *mock.MockauthService, validator *mockvalidation.MockValidator, req *http.Request) {
				...
				authSvc.EXPECT().
					Recover(gomock.Any(), gomock.Eq(expRecoverIn)).
			...
			assert: func(t *testing.T, res *httptest.ResponseRecorder, logs []string) {
				...
				assert.Len(t, logs, 1)
				expErrorLog := `{"level":"error", "error":"auth service dummy error", "message":"recover error on auth service"}`
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/recover-password", nil)
